...
class KNN_Classifier():
    ...
    def fit(self, train_descrs, train_labels):
        ...
        self.n = np.unique(train_labels).shape[0] #total number of classes in the training set
...
def tune_KNN(param_grid,train_descr,train_labels,val_descr,val_labels,verbose = True):
    #Hyperparameter tuning is performed based on the GAP metric
    ...
    for param_set in combinations:
        ...
        print(param_set)
        gap,_,_ = evaluate(val_preds,val_confs,val_labels)
        score = gap
        if score > best_score:
            best_score = score
