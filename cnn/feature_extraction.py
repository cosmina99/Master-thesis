...
#Extracting the image descriptors using a CNN model pre-trained on ImageNet.

def main():

	parser = argparse.ArgumentParser()
	parser.add_argument('directory', metavar='EXPORT_DIR')
	parser.add_argument('--net', default='r18INgem') #network used
	...
	if network_variant == 'resnet18':
		net = CNN("resnet18", gem_p = 3.0, pretrained_flag = True)
	elif network_variant == 'efficientnetb4':
		net = CNN("efficientnet_b4", gem_p = 3.0, pretrained_flag = True)
	elif network_variant == 'resnet50':
		net = CNN("resnet50", gem_p = 3.0, pretrained_flag = True)
	else:
